...
5. **触达范围**：一次 ingest 通常触达 **10-15 个 wiki 文件**（新建 + 更新）。这是正常的，不要怕改动多。
6. **更新 `wiki/index.md`**：把所有新建的页登记到对应 `domain × type` 分组（见 §10）。
7. **追加 `wiki/log.md`**：写一条 `ingest` 记录，含时间戳、源文件、触达的 wiki 文件列表、简要说明（见 §11）。
8. **检查矛盾**：对照步骤 4 的更新，看是否与既有编译真相冲突；有则按 §8 处理（时间线追加修正条目 + 重写编译真相 + 必要时 callout）。
9. **ingest 完成前 MUST 自检**（任一为否则不允许结束本次 ingest）：
