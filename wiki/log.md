...
## [2026-07-14 19:52] ingest | 新建 RAGFlow entity 页

- **源文件**：—（GitHub README，WebFetch 抓取，未落盘 raw/）
- **触达的 wiki 文件**：wiki/ragflow.md（新建 entity）, wiki/index.md（主题 MOC「知识管理 / LLM Wiki」登记）, wiki/log.md
- **说明**：新建 RAGFlow entity 页，记录 infiniflow 开源 deep document understanding RAG 系统。关键信息：agent-based RAG、知识库管理、7,355 commits、v0.26.4、Docker 部署。关联实体链至 [[llm-wiki]]、[[rag-vs-llm-wiki]]、[[weknora]]、[[gbrain]]。frontmatter sources 为空（未创建 raw 文件）。

## [2026-07-14 20:00] ingest | 新建 nashsu/llm_wiki entity 页 + 知识图谱 & 社区发现 concept 页

- **源文件**：raw/nashsu-llm-wiki-readme.md（新建；由 LLM 经 WebFetch 抓取 nashsu/llm_wiki README 落盘，用户授权）
- **触达的 wiki 文件**：wiki/nashsu-llm-wiki.md（新建 entity）, wiki/knowledge-graph.md（新建 concept）, wiki/community-detection.md（新建 concept）, wiki/rag-vs-llm-wiki.md（更新，新增生态位对比 5 列表格）, wiki/index.md（主题 MOC 登记）, wiki/log.md
- **说明**：基于 nashsu/llm_wiki README 创建 3 个新页：nashsu/llm_wiki entity 页（Two-Step CoT Ingest / 4-Signal KG / Louvain 社区发现 / Graph Insights / Persistent Queue / Auto-Watch 等 19 项特性）、知识图谱 concept 页（4-Signal 模型定义与对比）、社区发现 concept 页（Louvain 算法与 MOC 互补关系）。同步更新 rag-vs-llm-wiki.md 对比表从 2 列扩展为 5 列（RAG / LLM Wiki / GBrain / WeKnora / RAGFlow），新增 8 个维度行。

## [2026-07-14 20:00] ingest | 新建 GBrain & WeKnora entity 页

- **源文件**：—（GitHub README，WebFetch 抓取，未落盘 raw/）
- **触达的 wiki 文件**：wiki/gbrain.md（新建 entity）, wiki/weknora.md（新建 entity）, wiki/index.md（主题 MOC 登记）, wiki/log.md
- **说明**：新建 GBrain entity 页（Garry Tan / YC 总裁，146K 页 24K 人 5K 公司，synthesis layer / content-quality gate / RBAC / cron 自动化）和 WeKnora entity 页（腾讯，RAG+Agent+Auto-Wiki，hybrid RAG / multi-engine OCR / agentic RAG / MCP server）。frontmatter sources 均为空（未创建 raw 文件）。

## [2026-07-14 20:00] schema-update | 6 外部源交叉对照优化（spec cross-reference-6-external-sources）

- **源文件**：https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f, https://github.com/nashsu/llm_wiki, https://github.com/garrytan/gbrain, https://github.com/Tencent/WeKnora, https://github.com/infiniflow/ragflow, https://obsidian.md/zh/help
- **触达的 wiki 文件**：AGENTS.md, CLAUDE.md, wiki/llm-wiki.md, wiki/rag-vs-llm-wiki.md, wiki/karpathy-llm-wiki-gist-note.md, wiki/nashsu-llm-wiki.md, wiki/knowledge-graph.md, wiki/community-detection.md, wiki/gbrain.md, wiki/weknora.md, wiki/ragflow.md, wiki/index.md, wiki/log.md, raw/nashsu-llm-wiki-readme.md
- **说明**：对照 6 个外部资源（Karpathy LLM Wiki gist / nashsu/llm_wiki / GBrain / WeKnora / RAGFlow / Obsidian 文档）对仓库进行交叉优化，共 16 项任务：(1) Schema 修订——§1/§2 补「Obsidian 是 IDE / LLM 是程序员 / wiki 是代码库」核心隐喻与 schema co-evolution 哲学；§9.1 补 batch-ingest 方式 C 与 Two-Step CoT 分析阶段（步骤 0.5）；§9.3 补 Graph Insights 与社区检测检查项；§14 补 Obsidian 工具链（Web Clipper / 本地图片下载 / 四项核心价值）、Persistent Ingest Queue、Source Folder Auto-Watch。(2) 内容修复——对照 Karpathy 原文逐项修复 llm-wiki.md（补「为什么可行」段+Memex 1945+batch-ingest 提及）、rag-vs-llm-wiki.md（维护成本论证细节）、karpathy-llm-wiki-gist-note.md（补全 3 个缺失章节+1 条关键引文）。(3) 新建 6 个 wiki 页——nashsu-llm-wiki（entity）、knowledge-graph（concept）、community-detection（concept）、gbrain（entity）、weknora（entity）、ragflow（entity）。(4) 新建 1 个 raw 文件——nashsu-llm-wiki-readme.md。AGENTS.md 与 CLAUDE.md 已 diff 验证逐字一致。
