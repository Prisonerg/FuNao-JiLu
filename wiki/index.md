...
这是 wiki 主目录，包含两个视图：顶部主题 MOC（Map of Content）按主题聚合跨 type/domain 的相关页；下方按 domain × type 分组列出所有知识页。每次 ingest 后 LLM 会同步更新本目录；Query 操作时 LLM 会先读本目录定位相关页。链接用 Obsidian `[[wikilink]]` 语法（不带 `.md` 后缀）。
