...
- [ ] lint 脚本入链按唯一源页计数：重跑 lint，反向链接矩阵无同页重复列出（具体数字会随后续新页 ingest 而变，验证标准为「无重复」）
...
- [ ] `skills/llm-wiki-query/SKILL.md` 含回填流程 + 多形态输出（与 §9.2 一致；Task 20 瘦壳化后内容仍在）
...
- [ ] lint 新增检查 4：时间线条目缺 `§ 章节` 时警告（首条 ingest 豁免；对 15 页执行后报警告，补全后消失）
...
- [ ] 15 页时间线条目均含 `§ 章节` 锚点（lint 检查 4 对这些页不再报警告，首条豁免）
...
- [ ] `wiki/log.md` 末尾含一条 `schema-update` 说明本次时序重排（条目内容未改）
- [ ] `AGENTS.md §3` updated 字段说明明确「= 编译真相区最后一次重写日期，非 lint 触达日期」
- [ ] 20 页 updated 字段经核定，反映真实最后重写日期（14 页误刷的已回退，时间线含修正条目）
...
- [ ] `skills/llm-wiki-query/SKILL.md` 已瘦壳化（含 Task 8 回填流程 + 多形态输出）
...
- [ ] `AGENTS.md §14` 含「规模化搜索阈值」小节（>100 页引入 qmd，含 npm install + 8 阶段流水线说明）
- [ ] `AGENTS.md §14` 或 §9.1 含「entity 主动检测」策略
- [ ] CLAUDE.md 与 AGENTS.md 同步（§14 增补同步）

### 工具与格式

- [ ] `reports/` 目录已建；lint 详细报告写 `reports/lint-YYYY-MM-DD.md`，log.md 只记摘要
- [ ] lint 脚本支持 `--json` 参数输出 JSON
- [ ] log.md 条目格式为 `## [YYYY-MM-DD HH:MM] type | 简述`（历史条目已迁移，内容未丢）
- [ ] `AGENTS.md §11` 同步更新格式说明

### 新建 source-note 页

- [ ] `wiki/karpathy-llm-wiki-gist-note.md` 已建（source-note，ai 域，reliability: high）
...
- [ ] `wiki/uhpc-authoritative-standards-note.md` 已建（source-note，hobby 域，reliability: high）
- [ ] frontmatter 含 title/type/domain/tags/sources/created/updated/reliability（created=updated=2026-07-14）
- [ ] 含 `## 时间线` 双区结构
- [ ] 正文至少 1 条 `[[wikilink]]`（如 [[uhpc]]/[[steel-fiber-concrete]]）
- [ ] log.md 含对应 2 条 `ingest` 条目（source-note 补建）

### 新建 entity 页（低可靠性）

- [ ] `wiki/suda-ai-talk.md` 已建（entity，ai 域，reliability: low，title: "苏大讲AI"）
- [ ] frontmatter 含 title/type/domain/tags/sources/created/updated/reliability（created=updated=2026-07-14）
- [ ] 含 `## 时间线` 双区结构
- [ ] `## 关联实体` 段在 `## 时间线` 之上
- [ ] 正文至少 1 条 `[[wikilink]]`（如 [[suda-llm-wiki-video]]/[[llm-wiki]]）
- [ ] 编译真相区含 callout 标注「网络检索未找到权威背景，待后续 ingest 权威源升级」
- [ ] 时间线首条标注来源为「raw/suda-llm-wiki-douyin-2026-06.md + 网络检索」
- [ ] log.md 含对应 `ingest` 条目
- [ ] `wiki/nuan-nuan-planet.md` 已建（entity，personal 域，reliability: low，title: "暖暖小星球"）
- [ ] frontmatter 含 title/type/domain/tags/sources/created/updated/reliability（created=updated=2026-07-14）
- [ ] 含 `## 时间线` 双区结构
- [ ] `## 关联实体` 段在 `## 时间线` 之上
- [ ] 正文至少 1 条 `[[wikilink]]`（如 [[nuan-nuan-baby-cry-scratch-video]]/[[baby-cry-locate-itch]]）
- [ ] 编译真相区含 callout 标注「网络检索未找到权威背景，待后续 ingest 权威源升级」
- [ ] 时间线首条标注来源为「raw/nuan-nuan-baby-cry-scratch-douyin-2026-06.md + 网络检索」
- [ ] log.md 含对应 `ingest` 条目

### 修 andrej-karpathy.md 公众事实

- [ ] `raw/karpathy-biography-web-2026-07.md` 已落盘
- [ ] raw 文件头标注「本文件由 LLM 经 WebSearch 多源检索整理落盘，用户授权」+ 列出所有来源 URLs
- [ ] `wiki/andrej-karpathy.md` 时间线前 5 条（2015-2024）含完整 `YYYY-MM-DD` 日期
- [ ] 5 条时间线条目含 raw/ 来源（`raw/karpathy-biography-web-2026-07.md § 章节`）
- [ ] `wiki/andrej-karpathy.md` frontmatter sources 含新 raw 文件
- [ ] `wiki/andrej-karpathy.md` callout 已移除
- [ ] `wiki/andrej-karpathy.md` updated 已刷新为 2026-07-14
- [ ] log.md 含对应 `ingest` 条目（源文件：raw/karpathy-biography-web-2026-07.md）

### 新页登记

- [ ] 4 个新页出现在 index.md Dataview 渲染的 domain×type 分组（Task 19 已执行则自动渲染）
- [ ] 4 个新页通过 lint 检查（0 错误）
...
- [ ] 所有 26 项任务在 tasks.md 已勾选（P2 全不阻塞）
